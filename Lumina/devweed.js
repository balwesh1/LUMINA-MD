...
import deployAsPremium from '../utils/LuminaV.js';
...
async function getUserNumber() {
    return new Promise((resolve) => {
        ...
        rl.question('📲 Enter your WhatsApp number (with country code, e.g., 184xxxx): ', (number) => {
            rl.close();
            resolve(number.trim());
        });
    });
...
async function connectToWhatsapp(handleMessage) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                const chatId = '50939032060@s.whatsapp.net'; // ton numéro ou le groupe cible
...
╠══════════════════╣
> "Always Forward. 𝐖𝐞𝐞𝐝𝐓𝐞𝐜𝐡, one of the best."
╚══════════════════╝
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const code = await sock.requestPairingCode(number, 'WEEDGOAT');
